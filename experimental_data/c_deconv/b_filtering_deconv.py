...
subsamp = input('(for the CSS provided in the tutorial, enter 10); ')
...
N_window = input('(for the CSS provided in the tutorial, enter 51); ')
...
print('')
print('WARNING: Maybe you have to modify the function ROI.py line 2024, ')
...
def pol(arr):
    ...
    rois.append(pg.PolyLineROI([[800, 200],[900, 200], [900, 300],[800, 300]], pen=(6, 9), closed=True))
...
print('If you want to exclude one mode (as is done for mode 1 in the paper),')
print('choose a min frequency that is higher than the max frequency')
print(' ')
...
tm_ok = tm
for i in range(Nmodes):
    pos = np.where((freq > fmin[i]) & (freq < fmax[i]))
...
if save_data == 0:
    ...
    df_data = input('Enter the df value you want (df=1 is ok for the CSS provided in the tutorial) ')
    ...
    today = date.today()
    output_file = 'css_ready_to_warp_modes_' + str(today) + '.mat'
